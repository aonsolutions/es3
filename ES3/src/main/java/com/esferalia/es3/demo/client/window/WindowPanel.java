...
public final class WindowPanel extends FocusPanel {
  ...
  public WindowPanel(final WindowController windowController, Widget headerWidget,
      Widget contentWidget, boolean wrapContentInScrollPanel, final HandlerManager handler) {
	...
	close.setSize("30px", "30px");
	close.addClickHandler(new ClickHandler(){
		@Override
		public void onClick(ClickEvent event) {
			CloseEvent closeEvent = new CloseEvent();
			closeEvent.setwPanel(WindowPanel.this);
			handler.fireEvent(closeEvent);
		}
	});
